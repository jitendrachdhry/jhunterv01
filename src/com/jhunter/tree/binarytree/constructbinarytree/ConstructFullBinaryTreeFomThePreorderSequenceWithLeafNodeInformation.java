...
public class ConstructFullBinaryTreeFomThePreorderSequenceWithLeafNodeInformation {
    ...
    public static TreeNode constructTree(int[] preOrder, int isLeaf[]) {
        if (preOrder == null || preOrder.length <= 0 || isLeaf == null || isLeaf.length <= 0 || preOrder.length != isLeaf.length)
            return null;
    ...
    public static TreeNode constructTree(int[] preOrder, int[] isLeaf, AtomicInteger idx) {
        ...
        TreeNode node = new TreeNode(preOrder[idx.get()]);
        int isLeafNode = isLeaf[idx.get()];

        idx.incrementAndGet();
        if (isLeafNode == 0) {
            node.left = constructTree(preOrder, isLeaf, idx);
            node.right = constructTree(preOrder, isLeaf, idx);
        }
        return node;
    }
